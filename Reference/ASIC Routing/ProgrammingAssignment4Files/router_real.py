...
def iterator(ng,grid,Src,T,bend,via,bound):
	tags = ('N','S','E','W','U','D')
	Src_ret = []
	if len(Src) < 1:
		return 0
	...
	for S in Src:
		if (S == T):
			continue
		elif (S[1] < bound[0]) | (S[1] > bound[1]) | (S[2] < bound[2]) | (S[2] > bound[3]):
			continue
		else:
			l = S[0]
			x = S[1]
	...
	#print('Src : ' , Src_ret)
	#iterator(ng,grid,Src_ret,T,bend,via,bound)
	return Src_ret
...
def wavefront(grid,net,bend,via):
	...
	Src = [S]
	bound = (min(S[1],T[1])-10,max(S[1],T[1])+10,min(S[2],T[2])-10,max(S[2],T[2])+10)
	while len(Src) > 0:
		Src = iterator(ng,grid,Src,T,bend,via,bound)
	#print(ng)
